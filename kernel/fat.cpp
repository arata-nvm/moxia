...
size_t FileDescriptor::Write(const void *buf, size_t len) {
  ...
  if (wr_cluster_ == 0) {
    if (fat_entry_.FirstCluster() != 0) {
      ...
      fat_entry_.first_cluter_high = (wr_cluster_ >> 16) & 0xffff;
    }
  }
...
uint32_t ExtendCluster(uint32_t eoc_cluster, size_t n) {
  ...
  for (uint32_t candidate = 2; num_allocated < n; ++candidate) {
    if (fat[candidate] != 0) {
      continue;
    }
  ...
  if (const char *slash_pos = strrchr(path, '/')) {
    ...
    if (parent_dir_name[0] != '\0') {
      ...
      if (parent_dir == nullptr) {
        return {nullptr, MAKE_ERROR(Error::kNoSuchDirectory)};
      }
...
std::pair<DirectoryEntry *, bool> FindFile(const char *path, uint32_t directory_cluster) {
  if (path[0] == '/') {
