...
EFI_STATUS UefiMain(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *system_table) {
  ...
  if (EFI_ERROR(status)) {
    ...
    status = gBS->ExitBootServices(image_handle, memmap.map_key);
    if (EFI_ERROR(status)) {
      Print(L"Could no exit boot service: %r\n", status);
      Halt();
    }
  }
  ...
  typedef void EntryPointType(const struct FrameBufferConfig *, const struct MemoryMap *, VOID *);
  EntryPointType *entry_point = (EntryPointType *)entry_addr;
  entry_point(&config, &memmap, volume_image);
